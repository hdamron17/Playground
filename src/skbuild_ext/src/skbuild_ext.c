...
static struct PyModuleDef extModule = { PyModuleDef_HEAD_INIT, "skbuild_ext", ext_doc, -1, extMethods };

PyMODINIT_FUNC PyInit_skbuild_ext (void) {
    PyObject *m = PyModule_Create(&extModule);
    if (m == NULL)
        return NULL;

    ExtError = PyErr_NewException("skbuild_ext.error", NULL, NULL);
    Py_INCREF(ExtError);
